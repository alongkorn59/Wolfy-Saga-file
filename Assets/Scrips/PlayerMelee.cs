...
public class PlayerMelee : MonoBehaviour {
	...
	void Update () {
		if (Input.GetKeyDown(KeyCode.Z)) {
			//First off, let's check if we're not already punching,
			//see if the punch animations are playing on Layer 1 (The one with the arms).
			if(!CheckIfPlaying("RightPunch",1)&&!CheckIfPlaying("LeftPunch",1)) {
				...
				if(punchleft) {
					...
					playerMove.animator.Play("LeftPunch",1);
					punchleft=false;
					...
					playerMove.animator.Play("RightPunch",1);
					punchleft=true;
					StartCoroutine(WaitAndPunch());
				}
			}
